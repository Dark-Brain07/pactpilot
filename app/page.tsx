...
export default function Home(){return <main>
  <header><Link className="brand" href="/"><Image className="brandLogo" src="/pactpilot-logo.jpg" alt="" width={36} height={36} priority/>PactPilot</Link><nav><a href="#primitive">Primitive</a><a href="#safety">Safety</a><Link href="/monitor">Monitor</Link></nav><Link className="outline" href="/monitor">Open console</Link></header>
  <section className="hero"><div className="signal"/><div className="kicker"><span/> GENLAYER COMMERCIAL ASSURANCE</div><h1>Contracts should not<br/><em>go dark after signing.</em></h1><p>Turn bounded commercial clauses into living obligations, observed at anchored checkpoints and grounded in interval-verifiable evidence.</p><div className="actions"><Link className="primary" href="/monitor">Launch monitor <ArrowRight/></Link><a className="secondary" href="#primitive">See the mechanism</a></div><div className="statusRail">{states.map(([state,copy,tone])=><article key={state} className={tone}><small>{state}</small><b>{copy}</b></article>)}</div></section>
  <section id="primitive" className="section"><div className="sectionHead"><small>THE PRIMITIVE</small><h2>A temporal assurance graph,<br/>not an AI dispute court.</h2></div><div className="grid3"><article><FileSearch/><b>Bounded obligation</b><p>Human-approved clause digest, agreement version, counterparty and exact semantic requirement.</p></article><article><Clock3/><b>Anchored checkpoint</b><p>Each observation binds an anchored due time, sealed interval window, authority origin and durable historical SLA standing.</p></article><article><Workflow/><b>Append-only standing</b><p>Validators judge interval evidence. In-flight assessments remain preserved on closure; deterministic code controls provenance.</p></article></div></section>
  ...
  <footer><div className="brand"><Image className="brandLogo" src="/pactpilot-logo.jpg" alt="" width={36} height={36}/>PactPilot</div><p>Public evidence monitoring. Not legal advice or an automated damages engine.</p><Link href="/monitor">Monitor obligations →</Link></footer>
  </main>}
